...
public class GZIPFilter extends OncePerRequestFilter {
	...
	public void doFilterInternal(HttpServletRequest request,
		...
		if (isGZIPSupported(request)) {
			...
			if (log.isDebugEnabled()) {
				log.debug("GZIP NOT supported. " + request.getRequestURL());
			}
		}
	...
	private boolean isGZIPSupported(HttpServletRequest req) {
		...
		/*
		 * This needs to be turned off for certain types of content E.g. a csv
		 * export to Excel // disable gzip filter for exporting from displaytag
		 * 
		 * String exporting =
		 * req.getParameter(org.displaytag.tags.TableTagParameters.PARAMETER_EXPORTING);
		 * 
		 * if (exporting != null) { log.debug("detected excel export, disabling
		 * filter..."); return false; }
		 */
